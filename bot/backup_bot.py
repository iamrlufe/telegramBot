...
def _winscp_body(path: str, info: dict) -> str:
    ...
    if size_mb > WINSCP_BIG_LOG_MB:
        lines.append("")
        lines.append("⚠️ Журнал разросся: WinSCP пишет протокольный "
                     "обмен — по десятку строк на каждые 32 КБ данных. "
                     "На /loglevel=0 это остаётся; чтобы осталось только "
                     "важное, поставьте /loglevel=-1. Иначе на каждую "
                     "большую копию уходит под гигабайт диска.")
