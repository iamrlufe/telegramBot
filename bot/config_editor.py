...
📡 Пинг — сервер из списка или произвольный IP.

📋 Отчёт — по всем серверам. Только вручную: по расписанию вместо
   текста приходят графики бэкапов (08:00 и 18:00).
...
• 08:00 и 18:00 — два графика бэкапов: 🗂 свежесть по серверам и
  ...
  и команда /report
• воскресенье 09:00 — недельный отчёт + дайджест бэкапов
...
накапливается и никаких алертов отсюда не шлётся.
...
🔐 Ошибки входа — кто, с какого адреса и на какую базу не смог войти.
  Код state расшифрован словами: 5 — логина нет, 8 — неверный пароль,
  38/40 — нет доступа к базе, 18 — требуется смена пароля.
  Повторы с одного адреса схлопываются в строку со счётчиком.
💾 Ошибки бэкапа — текст ошибки движка (включая ошибку ОС и путь)
  плюс упавший шаг джоба Agent: что именно и где не сработало.
⚠️ Движок — severity 17+, повреждения (823/824/825), медленный
  ввод-вывод, взаимоблокировки. Штатный шум отфильтрован.
🕒 Джобы Agent — последние запуски всех джоб с итогом и длительностью.
  Видно и то, что джоб вовсе не стартовал.
📼 Копии из msdb — что сам SQL считает сделанным: база, тип, размер,
  путь. Расхождение с файлами на диске означает, что копию делал
  не SQL, и восстановление из неё не гарантировано.

━━━━━━━━━━━━━━━━━━━━
🔑 ПРАВА

Вход и Движок читают ERRORLOG — учётной записи мониторинга нужна роль
securityadmin (или sysadmin) на SQL. Джобы требуют SQLAgentReaderRole
в базе msdb, копиям из msdb хватает public. Если прав нет, раздел
пишет, какой роли не хватает; остальные разделы продолжают работать.

━━━━━━━━━━━━━━━━━━━━
⚠️ ЧЕГО ЖДАТЬ

Пустой список — не всегда «всё хорошо»: ERRORLOG обнуляется при
перезапуске службы SQL и по sp_cycle_errorlog. За 7 дней читаются
и архивные файлы, но если их нет, картина будет неполной.
Показываются свежие записи, глубина ограничена — журнал целиком
в сообщение не поместится."""
