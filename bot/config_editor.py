        ...
        if backups is not None:
            ...
            for btype, paths in backups.items():
                ...
                for p in plist:
                    ...
                    # Сверка с источником: опечатка в имени сервера или
                    # пути тихо выключила бы проверку — путь просто
                    # никогда не находился бы в БД.
                    mirror = p.get("mirror_of")
                    if mirror is not None:
                        if not isinstance(mirror, dict):
                            raise ValueError(
                                f"Сервер «{name}»: backups.{btype}.mirror_of — "
                                f"объект {{server, path}}"
                            )
                        if not mirror.get("server") or not mirror.get("path"):
                            raise ValueError(
                                f"Сервер «{name}»: backups.{btype}.mirror_of — "
                                f"нужны и server, и path"
                            )
                    for field, low, high in (("mirror_lag_minutes", 1, 1440),
                                             ("mirror_size_ratio", 0.1, 1.0)):
                        if p.get(field) is None:
                            continue
                        if mirror is None:
                            raise ValueError(
                                f"Сервер «{name}»: backups.{btype}.{field} "
                                f"без mirror_of ничего не делает"
                            )
                        try:
                            value = float(p[field])
                        except (TypeError, ValueError):
...
PRESERVED_PATH_KEYS = ("schedule_weekday", "schedule_by_hour", "size_check",
                       "ignore_logs", "mirror_of", "mirror_lag_minutes",
                       "mirror_size_ratio")
...
def path_details(field: str, item) -> list:
    ...
    # Сверка с источником задаётся только в servers.json, но молчать о ней
    # в карточке нельзя: иначе непонятно, откуда берётся «БЭКАП НЕ ДОЕХАЛ».
    mirror = data.get("mirror_of")
    if isinstance(mirror, dict) and mirror.get("server"):
        lines.append(f"   🔗 сверка с: {mirror['server']} {mirror.get('path', '')}".rstrip())
        lag = data.get("mirror_lag_minutes")
        lines.append(f"   ⏳ ждать копию: {lag} мин" if lag
                     else "   ⏳ ждать копию: общий порог")
    return lines
...
HELP_MIRROR = """🔗 КОПИИ ПРИЕЗЖАЮТ С РЕГИОНА

Про каталог, куда копии не создаются, а ПРИЕЗЖАЮТ с другого сервера
(приёмник, куда регионы заливают файлы по SFTP/FTP).

Обычный порог «БЭКАП УСТАРЕЛ» тут поздний: чтобы не звенеть на каждой
задержке копирования, его ставят с запасом, и о непривезённой копии
узнаёшь через сутки.

Источник знает точнее: файл на регионе создан в 00:34 — значит задание
отработало, и копия обязана появиться. Не появилась — сбой копирования,
и сказать об этом можно через 5 минут, а не назавтра.
...
mirror_lag_minutes — сколько ждать копию, прежде чем звонить. Крупная
база копируется минутами, без запаса тревога звенела бы каждую ночь.

Мастер ⚙️ Настройка эти поля не спрашивает, но и НЕ теряет при правке
других настроек пути. В карточке пути видно строкой «🔗 сверка с:».

━━━━━━━━━━━━━━━━━━━━
🔔 ЧТО ПРИХОДИТ

🆘 БЭКАП НЕ ДОЕХАЛ — на источнике файл свежее всего, что есть здесь,
   и с его создания прошло больше mirror_lag_minutes.
🆘 КОПИЯ ПРИЕХАЛА НЕ ЦЕЛИКОМ — файл на месте, но меньше оригинала.

Второе важнее, чем кажется: по SFTP оборванная загрузка выглядит для
сервера УСПЕШНОЙ — протокол не сообщает ожидаемый размер, сервер видит
только закрытие соединения. Поймать огрызок можно лишь сравнением с
оригиналом. По той же причине сверка работает там, где бесполезна
обычная проверка размера: у DIFF-копий размер растёт всю неделю, и
сравнивать с историей нечего.

━━━━━━━━━━━━━━━━━━━━
🤫 КОГДА МОЛЧИТ

• на источнике нет копий вообще — это его собственная авария, о ней
  скажет проверка самого источника, дважды об одном не звеним;
