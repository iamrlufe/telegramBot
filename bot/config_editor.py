...
Находка говорит «отправитель запрещён», а не «подделка», и это
не осторожность формулировки: текст отказа у Postfix стандартный
и одинаковый у карты антиспуфинга и у чёрного списка. Из лога они
неразличимы. Чтобы считать ровно подделку, нужно дать своим
записям своё сообщение в карте и вписать его в
ZIMBRA_SENDER_REJECT_MARK. Остальные отказы (грелист, обратный DNS,
спам-листы) в этот счёт не идут и не будят — это фон интернета.

Пороги — в .env (ZIMBRA_*). Алерты проходят через общий механизм:
mute, тихие часы, «Принято», утренняя сводка.

Тот же проход журналов кладёт сводку в базу для вкладки 📮 Почта в
📊 Дашборде: письма за сутки, очередь, отбитые, неудачные входы и
списки. Второго чтения лога для этого не делается. В списках
MAIL_SUMMARY_ROWS строк (по умолчанию 15, потолок 20).

Обзор дашборда показывает не всё — из него убрано то, что вытесняло
полезное: служебные входы самого сервера (без адреса в логе), входы
из локальной сети и служебные отправители («<>» — отчёты о
недоставке, MAILER-DAEMON, postmaster). Если снаружи не заходил
никто, раздел говорит об этом строкой, а не исчезает.

Фильтры действуют только на успешные входы и только в дашборде.
Неудачные попытки видны все — подбор пароля из своей же сети
означает заражённую машину внутри периметра. Полные списки без
фильтров — здесь, в 📤 Отправители и 🔑 Входы.
