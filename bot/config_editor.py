...
Обзор дашборда показывает не всё: убраны служебные входы сервера
(без адреса в логе), входы из локальной сети и служебные
отправители («<>», MAILER-DAEMON, postmaster). Если снаружи не
заходил никто, раздел говорит об этом строкой, а не исчезает.
Отправители разделены на «Кто отправляет» (сотрудники) и «Кто
пишет вам» (чужие рассылки); делит происхождение письма, а не
адрес — свой домен в конверте подделывается свободно.

Фильтры — только для успешных входов и только в дашборде.
Неудачные видны все: подбор пароля из своей же сети означает
заражённую машину внутри периметра. Полные списки — здесь,
в 📤 Отправители и 🔑 Входы.
