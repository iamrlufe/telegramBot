...
4. 💾 Бэкапы → 📤 Копирование → «📤 Скопировать сейчас». Проверь, что
   скрипт стартует и файл приезжает; там же видно, сколько рейс шёл —
   по этому числу ставь «Копирование зависло» с запасом.
...
💾 Бэкапы → 📤 Копирование: видно, у кого копированием управляет бот,
чем он это делает и когда был последний рейс. Там же кнопки запуска —
своя на каждый тип копии: «📤 Полная», «📤 Разностная», «📤 Журнал».
Если источников несколько, бот сперва спросит сервер. Права нужны те
же, что на удаление файлов.

Пока идёт один рейс, второй на том же сервере не запустится: перед
запуском бот смотрит на сервере, не крутится ли уже WinSCP с этим
скриптом.
Полная и разностная стоят в очереди каждая своя: увезли разностную —
полная поедет следующим циклом. Ручной запуск не сбивает обычный ход.
