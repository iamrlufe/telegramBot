...
замолкает: доходить нечему. Тогда считаются отбитые попытки —
порог ZIMBRA_SENDER_REJECT_ALERT, по умолчанию 50 за сутки. Он выше
намеренно: отбивается каждая попытка, а доходило только то, что
проскочило. Остальные отказы (грелист, обратный DNS, спам-листы) в
этот счёт не идут и не будят — это фон интернета.
