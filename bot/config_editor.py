...
• мало места на диске: свободно < 15% / 10% / 5%
  ...
  Служебные разделы ядра Linux (/sys, /proc, /dev, /run) и всё
  меньше 1 ГБ дисками не считаются: efivars и подобные заполнены
  по устройству и висели бы вечным «0% свободно». Диск, метрики
  которого перестали приходить, пропадает из отчёта через 2 часа
