...
def validate_config(servers) -> None:
    ...
    for i, server in enumerate(servers, 1):
        ...
        # Автобан без разрешения блокировать — настройка, которая молча
        # ничего не делает.
        if server.get("autoban_brute") and not server.get("firewall"):
            raise ValueError(
                f"Сервер «{name}»: autoban_brute без firewall ничего не "
                f"блокирует — включите блокировку IP"
            )
        ...
        # Процент считается по паре «сервер + корень»: одно без второго
        # молча ничего не показывает.
        has_target = bool(str(server.get("copy_target") or "").strip())
        has_root = bool(str(server.get("copy_target_root") or "").strip())
        if has_target != has_root:
            raise ValueError(
                f"Сервер «{name}»: copy_target и copy_target_root задаются "
                f"только вместе — по одному из них процент не посчитать"
            )
        if has_target and not script:
            raise ValueError(
                f"Сервер «{name}»: приёмник задан, а copy_script — нет"
            )
        ...
        for flag in ("dbsize", "exchange", "firewall", "zimbra",
                     ...
                     "legacy_tls", "copy_after_backup", "autoban_brute"):
...
FIELD_DEFS = {
    ...
    "autoban_brute": {
        "label": "Автобан перебора",
        "prompt": "Блокировать самому тех, кто перебирает пароли к почте? "
                  "(да/нет)\n"
                  "Блокируются только адреса, у которых страна ИЗВЕСТНА и "
                  "не домашняя (ZIMBRA_HOME_COUNTRY, по умолчанию KZ). "
                  "Неизвестная страна — не повод: такой адрес остаётся "
                  "кнопкой под алертом.\n"
                  "Нужен флаг блокировки IP: без него блокировать нечем.\n"
                  "Пропусти — не блокировать самому (значение по умолчанию).",
        "kind": "bool",
    },
    "copy_script": {
    ...
    "copy_target": {
        "label": "Приёмник копий",
        "prompt": "Имя сервера-ПРИЁМНИКА, как оно записано в конфиге бота "
                  "(тот, куда скрипт заливает копии по SFTP).\n\n"
                  "Нужно ровно для одного — показывать процент готовности: "
                  "у SFTP нет обратной связи о ходе передачи, сколько "
                  "доехало, знает только приёмник.\n"
                  "Пропусти — процент показываться не будет, остальное "
                  "работает.",
        "kind": "text",
    },
    "copy_target_root": {
        "label": "Каталог на приёмнике",
        "prompt": "Каталог на приёмнике, соответствующий корню SFTP.\n"
                  "Например: E:\\Backups\\AKT1C8\n\n"
                  "Путь из журнала (/new_pro_akt/FULL/файл.bak) бот "
                  "приклеит к нему и посмотрит размер растущего файла.",
        "kind": "text",
    },
    "copy_delay_minutes": {
...
TOGGLE_FIELDS = ["dbsize", "exchange", "firewall", "zimbra", "verify_backup",
                 "backup_size_check", "copy_after_backup", "autoban_brute"]
...
def _autoban_row(server: dict) -> list:
    """Ряд с автобаном перебора — только когда блокировка вообще разрешена.
    Без флага firewall блокировать нечем, и кнопка обещала бы несбыточное."""
    if not server.get("firewall"):
        return []
    return [[InlineKeyboardButton(
        f"🛡 Автобан перебора: {'✅' if server.get('autoban_brute') else '❌'}",
        callback_data="cfg_toggle:autoban_brute"
    )]]
...
def edit_fields_kb(server: dict):
    ...
    if server_type == "windows":
        ...
        # Автозапуск копирования показываем, только когда скрипт задан:
        # без скрипта переключать нечего, а кнопка сбивала бы с толку.
        if server.get("copy_script"):
            size_row.append(InlineKeyboardButton(
                f"📤 Копировать сам: "
                f"{'❌' if server.get('copy_after_backup') is False else '✅'}",
                callback_data="cfg_toggle:copy_after_backup"
            ))
        keyboard.append(size_row)
        keyboard += _autoban_row(server)
...
📊 Дашборд — вся инфраструктура одним HTML-файлом: кольцо свободного
   ...
   • Windows и SQL — журналы за сутки: счётчики по категориям
     ...
     живьём при открытии ничего не читается. У SQL видно и
     запуски джоб Agent, и те, что НЕ отработали: расписание
     читается из msdb, поэтому «должна была и не запустилась»
     видно наравне с падениями. Выключенные джобы там же.
...
• раз в полчаса — почта Zimbra (ZIMBRA_SCAN_MINUTES): находки для
  алертов и сводка для вкладки 📮 Почта одним чтением журналов
• раз в час — почта Exchange (EXCHANGE_SCAN_MINUTES): OWA, ActiveSync
  и отказы входа для той же вкладки. Алертов тут нет: подбор пароля
  к OWA виден в журнале Security и разбирается сводкой Windows
...
🧪 Verify статус — результаты RESTORE VERIFYONLY за 7 дней и ручной
   ...
   ботом можно пользоваться, не дожидаясь). Сверка страниц
   (WITH CHECKSUM) включается, только если копия снята с
   контрольными суммами: на копии без них такая проверка не
   предупреждает, а падает. Суммы задаются в задании бэкапа.
...
Сигнал — именно запись о копии, а не задание SQL Agent: сколько бы
джоб ни стояло на сервере, реиндекс и checkdb копирование не запустят.
А если на сервере в этот момент делается копия другой базы, рейс
откладывается: увозить каталог, пока в него пишут, нельзя.

━━━━━━━━━━━━━━━━━━━━
▶️ КАК ВКЛЮЧИТЬ

1. ⚙️ Настройка → ✏️ Изменить сервер → сервер-ИСТОЧНИК (где MSSQL
   и где лежит готовый скрипт копирования).
2. «Скрипт копирования» → полный путь на самом сервере. Управление
   включилось, появился переключатель «📤 Копировать сам».
3. Нажми «📤 Копировать сам», чтобы стало ❌ — сперва обкатаем руками.
4. 💾 Бэкапы → 📤 Копирование → сервер → «📤 Скопировать сейчас».
   Проверь, что скрипт стартует и файл приезжает; там же видно,
   сколько рейс шёл — по этому числу ставь «Копирование зависло».
5. Верни «📤 Копировать сам» в ✅ и ВЫКЛЮЧИ задание в планировщике
   Windows — иначе копирование пойдёт дважды.
...
📜 Скрипт копирования — путь спрашивается по шагам: сперва для
   ПОЛНОЙ (обязательно), потом для РАЗНОСТНОЙ и ЖУРНАЛА — эти два
   пропускаются кнопкой. Пути на самом сервере (.ps1, .bat, .cmd,
   .exe), типы берутся из самих скриптов. «-» на первом шаге
   убирает управление копированием совсем.
🔁 Копировать автоматически — можно временно выключить, не стирая путь.
💾 Какие копии возить — D полная, I разностная, L журнал.
   По умолчанию D,I: журналы делают каждые 15–60 минут, скрипт
...
📥 Приёмник копий — бот спросит имя приёмника и каталог на нём.
   Только ради процента: у SFTP нет обратной связи о ходе передачи,
   сколько доехало, знает лишь приёмник. Он должен быть заведён у
   бота как сервер (размер смотрится по WinRM, не по SFTP). Задал —
   у едущей базы появится шкала: ▰▰▰▱▱▱▱▱▱▱ 32%, 14.26 ГБ, ещё ≈2 ч.
...
💾 Бэкапы → 📤 Копирование — СПИСОК источников: значок говорит, чем
сервер занят (⏳ идёт, ✅ прошёл, ❌ не удался, ⏸ рейсов не было).
Кнопок запуска в списке нет намеренно: запуск поверх идущего рейса
портит файл на приёмнике.

Заходишь в сервер — там всё по нему: кнопки запуска (своя на каждый
тип: «📤 Полная», «📤 Разностная», «📤 Журнал»; скрипт один на все
типы — одна «📤 Скопировать сейчас»), «📄 Журнал скрипта» и
«⏹ Сбросить рейс» у идущего рейса. Права те же, что на удаление.

Второй рейс на том же сервере не запустится, пока идёт первый: бот
смотрит, не крутится ли уже WinSCP с этим скриптом. Ручной запуск не
сбивает обычный ход.

━━━━━━━━━━━━━━━━━━━━
🧩 ЭТО ОПЦИЯ

Не задан скрипт — ничего не меняется: копии возит планировщик Windows,
бот только смотрит за каталогами, как раньше. Включать можно по одному
серверу, и «Копировать автоматически: нет» выключает автозапуск, не
стирая настройку.

Одна копия отправляется один раз. Копию старше трёх часов бот не везёт
(защита после перезапуска) — но ту, что ждала очереди за идущим рейсом,
везёт: за многочасовой полной разностная успевает состариться."""


HELP_AUTOBAN = """🛡 АВТОБАН ПЕРЕБОРА ПОЧТЫ

Бот сам блокирует тех, кто перебирает пароли к почтовым ящикам.

Почему здесь можно, а для сайта нельзя. У почтового перебора признаки
узкие: это неудачные ВХОДЫ по SMTP/IMAP с чужого адреса, а не запросы
к сайту. Обратный прокси тут ни при чём, и свой сотрудник с
индонезийского адреса в почту не ходит.

━━━━━━━━━━━━━━━━━━━━
🎯 КОГО БАНИТ

Три условия, и все три обязаны совпасть:

1. находка — именно перебор пароля (обычный с одного адреса или
   распылённый: по одной попытке с десятков адресов, порог по
   одному адресу такое не берёт);
2. адрес внешний — частные сети не блокируются никогда;
3. страна адреса ИЗВЕСТНА и не домашняя (по умолчанию KZ).

Третье — главная защита. Неизвестная страна значит «не удалось
определить», а не «чужой»: такой адрес бот не трогает, он остаётся
вам кнопкой «🛡 Заблокировать» под тем же алертом.

Всплеск отправки, подделка отправителя, очередь — не повод банить,
на них метки нет.

━━━━━━━━━━━━━━━━━━━━
⚙️ КАК ВКЛЮЧИТЬ

⚙️ Настройка → сервер почты:
1. включить «🛡 Блокировка IP» — без неё блокировать нечем;
2. появится «🛡 Автобан перебора» — включить его.

По умолчанию ВЫКЛЮЧЕНО: молча отрезать адреса бот не должен.

━━━━━━━━━━━━━━━━━━━━
🔔 ЧТО ВИДНО В АЛЕРТЕ

Список заблокированных адресов целиком, без сокращений, с флагом
страны у каждого. Кнопки «Заблокировать» остаются только у тех, кого
бот не тронул, — дублировать уже сделанное незачем.

За один заход банится не больше 20 адресов; остаток назван в алерте
и блокируется вручную. Повторный проход по тому же перебору адрес
второй раз не банит.

━━━━━━━━━━━━━━━━━━━━
↩️ КАК СНЯТЬ

Ничего своего автоблокировка не заводит: на Linux это обычная клетка
fail2ban (снимется и сама, по её сроку), на Windows — правило фаервола
на 7 дней. Снять раньше — раздел 🛡 в карточке сервера, как всегда."""


HELP_COPY_FAIL = """🚑 КОПИРОВАНИЕ: КОГДА НЕ ПОЛУЧИЛОСЬ

🆘 КОПИРОВАНИЕ НЕ ЗАПУСТИЛОСЬ — скрипт не стартовал. Файл готов, но
   никуда не поедет: нужно запускать руками.
🆘 КОПИРОВАНИЕ НЕ УДАЛОСЬ — скрипт отработал с ошибкой. В сообщении
   код возврата и конец журнала: обычно там сразу видно причину —
   нет прав, не найдена сессия WinSCP, не смонтирован диск. Такой
   рейс не считается отправленным, следующий цикл попробует снова.
🆘 КОПИРОВАНИЕ ЗАВИСЛО — процесс жив дольше порога.

Весь вывод скрипта (консоль: stdout и stderr) пишется на сервере в
%ProgramData%\\bot\\copy — путь к журналу последнего рейса
виден в разделе 📤 Копирование. Собственный лог скрипта это не
отменяет: в файле бота есть и то, чего в нём быть не может, —
сообщения оболочки о том, что запуск не состоялся вовсе.
Файлы старше 30 дней бот удаляет сам.

Кракозябры в журнале — это две кодировки в одном файле: cmd пишет
в кодировке консоли (866 или 1251), а WinSCP — в UTF-8. Бот читает
все три построчно, менять ради этого в скрипте ничего не нужно.

📄 Журнал — читает журнал ВАШЕГО скрипта и показывает разбор: сколько
баз, что залито и за сколько, что пропущено, где были ошибки.
✅ залито (скрипт сказал SUCCESS) · ⏭ уже было на приёмнике ·
⏳ ещё в пути · ❌ не удалось. Кнопка на каждую базу
открывает журнал WinSCP: он протокольный, поэтому бот показывает из
него только значимое — время последней записи, идёт ли передача,
ошибки. Путь бот вычисляет сам: рядом со скриптом каталог logs,
в нём каталог на дату.

❗ ОГРЫЗОК НА ПРИЁМНИКЕ

Знак ❗ значит: скрипт счёл базу залитой или пропущенной, а файл на
приёмнике короче домашнего. Обрыв заливки по SFTP выглядит успехом,
а сверка размеров в cmd от этого не спасает: IF EQU насыщается на
2 147 483 647, и любые два файла больше 2 ГиБ для него РАВНЫ. Проверьте
сами: if 3000000000 EQU 4000000000 (echo РАВНЫ) else (echo разные).

Сам он это не исправит: пока обрезок лежит на месте, база будет
«пропускаться» каждый рейс. Удалите файл на приёмнике — следующий
рейс увезёт копию заново. В скрипте сравнивайте размеры строками
(if "!R!"=="!L!") и при сомнении заливайте, а не пропускайте.

Бот сверяет это сам, если задан «Приёмник копий».

«Файла нет» по базе — обычно не поломка. Этот журнал ведёт WinSCP,
и появляется он, только когда базу реально заливали. У базы со
знаком ⏭ (файл уже был на приёмнике) заливки не было, писать его
было нечем. Ход рейса целиком — в общем журнале, кнопкой «◀️ Назад».

⏳ «ИДЁТ», А НА СЕРВЕРЕ ТИХО

Рейс закрывает монитор, поэтому раздел 📤 Копирование при открытии
сам спрашивает сервер про рейсы, которые числятся идущими. Рейс,
где все базы уже на приёмнике, кончается за полминуты — без этого
он показывался бы «идёт» до следующего цикла монитора.

Удачный рейс раздел закрывает сам, неудачный оставляет монитору:
тревогу по нему шлёт он, и молчаливое закрытие оставило бы всех
без неё. Про такой рейс так и написано — уже кончился неудачно,
монитор разберёт в ближайшем цикле.

🛡 ПОЧЕМУ БОТ ОТКАЗЫВАЕТСЯ ЗАПУСКАТЬ

«Копирование этим скриптом уже идёт на сервере (WinSCP.com PID …)» —
предохранитель. Проверка идёт по факту, а не по памяти: состояние
может разойтись с действительностью, а две программы, дописывающие
один файл на приёмнике, — худшее, что может случиться с копией.
Ищутся и обёртка cmd, и сам WinSCP: у второго в командной строке
скрипта нет, поэтому он опознаётся по каталогу журнала. Осиротевший
WinSCP (родителя сняли, он остался) — самый опасный случай.

Если спросить сервер не удалось, запуск тоже отменяется: цена ошибки
здесь выше цены задержки.

Если рейс числится идущим, а на сервере его нет (убили процесс,
перезагрузили сервер) — кнопка «⏹ Сбросить рейс». Она не убивает
ничего на сервере, а лишь перестаёт считать его занятым: без неё
...
Образец самого скрипта — в репозитории, каталог scripts/.
"""
...
Вкладка 📮 Почта в 📊 Дашборде показывает ту же почту по всем серверам
сразу — и Exchange, и Zimbra. Там данные готовые: монитор собирает их
раз в EXCHANGE_SCAN_MINUTES (по умолчанию час) и кладёт в базу. Живьём
логи читает только этот раздел карточки — на все серверы разом это
минуты ожидания при каждой рассылке дашборда.

В обзоре дашборда скрыты пробы самой Exchange: ящики HealthMailbox
и SystemMailbox, агенты AMProbe и TestActiveSyncConnectivity.
Managed Availability дёргает ими каждый протокол раз в минуту, и они
занимали треть списка телефонов. Здесь, в карточке, списки полные:
если проверки перестали ходить, это тоже новость.

Поток писем в дашборде берётся из логов трассировки сообщений —
родного CSV Exchange. Оттуда сколько принято и отправлено, кто
отправляет, кто пишет вам, что не доставлено и длина очереди.
Письма считаются по message-id: одно письмо даёт несколько событий
транспорта, и счёт строк завысил бы объём в разы. Отбитых на входе
там нет — такие письма в транспорт не попадают, они остаются в
протокольных логах, а те по умолчанию выключены.
...
🔎 Сканирование — запросы мимо публикаций, кто и куда стучится и
  ...
  поисковые роботы. Свои пути тоже: виртуальный каталог и папка в
  корне сайта публикациями не считаются, но содержимое по ним
  сервер отдаёт законно, и находкой это не будет.
...
Запросы из своих сетей (10.x, 172.16–31.x, 192.168.x, 100.64.x,
127.x, 169.254.x) в алерты и в 🚨 Проблемы не идут: за внутренним
адресом сотрудник, свой сканер безопасности или мониторинг. В
разделах ниже такие строки остаются, тревог по ним нет.
...
HELP_SQLJOBS = """🕒 ДЖОБЫ SQL AGENT

Две разные картины, и путать их нельзя.

━━━━━━━━━━━━━━━━━━━━
🕒 ЧТО ЗАПУСКАЛОСЬ (кнопка в карточке сервера)

Последние запуски с итогом и длительностью, читаются в момент нажатия.
Видно и то, что джоб вовсе не стартовал. Пустая история чаще означает
не остановленный Agent, а нехватку прав: без роли SQLAgentReaderRole
в msdb учётная запись видит только свои джобы, чужие молча не попадают
в выборку. Раздел сам подсказывает, какой из двух случаев произошёл.

📼 Копии из msdb — что сам SQL считает сделанным: база, тип
(Full/Diff/Log), размер, путь. Расхождение с файлами на диске значит,
что копию делал не SQL, и восстановление из неё не гарантировано.

━━━━━━━━━━━━━━━━━━━━
▶️ ЗАПУСКИ ЗА СУТКИ (вкладка SQL в дашборде)

Строка на джобу: сколько раз отработала, как долго шла дольше всего,
сколько раз упала, бэкапная ли она. Дольше SQL_JOB_LONG_HOURS (12 ч) —
пометка: успех, но ночное окно съедено. В алерты это не идёт.

━━━━━━━━━━━━━━━━━━━━
🗓 ЧЕГО НЕ БЫЛО

Не отработавшая джоба не падает и в истории не появляется вовсе, поэтому
расписания читаются из msdb: видно «должна была и не запустилась».
Разбираются ежедневная, недельная и «N-го числа»; однократная и «при
старте Agent» — нет, по ним пропуск не угадать без ложных тревог.

Идущая сейчас джоба пропущенной не считается: запись о запуске
появляется только при завершении, и семичасовой ночной план в пять утра
ещё не отчитался. Плюс запас SQL_JOB_MISS_GRACE_MINUTES (час) после
назначенного времени: джоба стартует не секунда в секунду.

Выключенные джобы — там же. Самый тихий случай: не падает, не пропадает,
не жалуется, а копий от неё не будет.

━━━━━━━━━━━━━━━━━━━━
🔑 ПРАВА

История запусков требует SQLAgentReaderRole в базе msdb.
Расписания берутся из sysjobs/sysschedules — хватает public."""
...
По находкам сайта бот НЕ блокирует никого сам. Адрес называете вы —
посмотрев на 🌐 IIS → 🔎 Сканирование. Автоматика там дороже пользы:
на Exchange один неверно опознанный адрес это отрезанный офис, а если
сайт за обратным прокси, в логе у всех посетителей один и тот же адрес
прокси — автоблокировка выключила бы сайт целиком.

Исключение — перебор паролей к почте: там признаки узкие (неудачные
ВХОДЫ с чужого адреса), и бот может банить сам. Переключатель
«🛡 Автобан перебора» в карточке сервера, подробности — раздел справки
🛡 Автобан перебора почты.

━━━━━━━━━━━━━━━━━━━━
📑 ЧТО ЕСТЬ

🔎 Кандидаты на блокировку — бот сам собирает список из сводки
  IIS за сутки, вы отмечаете нужные кнопками и жмёте «Заблокировать».
  Переписывать адреса руками не нужно. Та же кнопка есть в 🌐 IIS —
  «🛡 Заблокировать сканеров», прямо рядом со списком сканирующих.
  В список идут три повода: сервер отдал содержимое по постороннему
  пути, идёт подбор пароля 1С, с адреса пришло больше 100 посторонних
  запросов за сутки. Не идут: уже заблокированные, белый список, узлы
  Cloudflare, свои и внутренние адреса. Срок общий на всю пачку,
  кнопка «Срок» перебирает 3 / 7 / 30 дней и «бессрочно».
🚫 Заблокировать — адрес или подсеть вручную, со сроком:
...
Два источника, и это не прихоть. Про входы postfix-лог знает почти
ничего: люди работают через веб и mailboxd, а postfix получает
готовое письмо — sasl_username= попадается там единицы раз. Кто и
откуда заходил, знает /opt/zimbra/log/audit.log.
...
🔑 Входы — учётки, адреса со странами, подбор пароля, входы из-за
  границы. Протокол берётся из oproto= (smtp, imap): protocol= в
  логе почти всегда soap, потому что пароль проверяет mailboxd.
  Пометка «админ-консоль» ставится только по порту 7071 — путь
  /service/admin/ общий у админки и у проверки пароля SMTP.
...
🔴 подбор пароля: неудачные попытки с адреса на одну учётку —
  в тексте указано, чем ломятся (smtp — хотят рассылать спам от
  вашего имени, imap — читать почту);
🟠 распылённый перебор: одна учётка с нескольких адресов сразу
  (ZIMBRA_SPRAY_ADDRESSES_ALERT, по умолчанию 3). Порог по одному
  адресу такой перебор не берёт — по одной попытке с полусотни
  адресов, и так его обходят. Краснеет, если с тех же адресов
  вход в итоге удался;
🔴 письма своей учётки сданы С БЕЛОГО АДРЕСА ПО ПАРОЛЮ — здесь все
  пишут через веб, а служебные учётки сдают почту с внутренних
  адресов; сдача снаружи не описывается ни тем, ни другим.

Про пароль здесь не для красоты. Один и тот же smtpd принимает почту
из интернета на 25-й порт и сдачу пользователя на 587-й, строка
client= у них одинаковая, а отправитель в конверте у входящего
письма любой — свой домен подделывается свободно. Без этого условия
раздел выдавал 🔴 «угнана учётка» на обычный спам.

Подделка отправителя, всплеск отправки и раздутая очередь идут
как 🟠. Подделка — одной строкой на сервер, с порогом
ZIMBRA_SPOOF_ALERT: письмо от «коллеги» убедительнее любого фишинга,
и раз оно дошло, SPF/DMARC его не отбили.

Если подделку режет сам Postfix (письмо с вашим доменом в конверте
от чужого IP отбивается с «Sender address rejected»), правило выше
замолкает: доходить нечему. Тогда считаются отбитые отказы —
порог ZIMBRA_SENDER_REJECT_ALERT, по умолчанию 20 за сутки.

Находка говорит «отправитель запрещён», а не «подделка»: текст
отказа у Postfix одинаков у карты антиспуфинга и у чёрного списка,
из лога они неразличимы. Чтобы считать ровно подделку, дайте своим
записям своё сообщение и впишите его в ZIMBRA_SENDER_REJECT_MARK.
Прочие отказы (грелист, обратный DNS) в счёт не идут — это фон.

Пороги — в .env (ZIMBRA_*). Алерты проходят через общий механизм:
mute, тихие часы, «Принято», утренняя сводка.

Тот же проход журналов кладёт сводку в базу для вкладки 📮 Почта
в 📊 Дашборде — про неё отдельный раздел справки, «📮 Zimbra в
дашборде». Второго чтения лога для этого не делается.
...
HELP_ZIMBRA_DASH = """📮 ZIMBRA В ДАШБОРДЕ

Вкладка 📮 Почта в 📊 Дашборде: плитки (письма за сутки, очередь,
отбитые, неудачные входы) и списки. Данные берутся готовыми из базы,
живьём журналы не читаются — их уже прочитал монитор ради алертов.

В каждом списке MAIL_SUMMARY_ROWS строк, по умолчанию 15. Потолок —
20: столько групп вообще приезжает с сервера, просить больше нечего.

━━━━━━━━━━━━━━━━━━━━
👤 КТО ОТПРАВЛЯЕТ И КТО ПИШЕТ ВАМ

Два списка, а не один. Postfix считает всех, кто прошёл через сервер,
и в общем списке верх занимали чужие рассылки — банк, налоговая,
доски вакансий, — а всплеск отправки у своей учётки терялся.

Делит не адрес, а происхождение письма: свой — это адрес вашего
домена И письмо, родившееся на сервере (веб) или сданное изнутри по
паролю. Свой домен в конверте подделывается свободно, поэтому спам,
притворяющийся сотрудником, уходит во входящие.

Служебные отправители убраны: пустой конверт «<>» (так postfix шлёт
отчёты о недоставке), MAILER-DAEMON, double-bounce, postmaster.

━━━━━━━━━━━━━━━━━━━━
🔑 КТО ЗАХОДИЛ

Убраны служебные обращения самого сервера (у них в логе нет адреса)
и входы с рабочих мест локальной сети: ими заполнялся весь список, и
вход снаружи в него не попадал.

Входы через шлюз остаются. Если сеть ходит в почту через
маршрутизатор, все приходят с его адреса, и внешний сотрудник
неотличим от сидящего рядом — настоящий адрес до сервера не доезжает.
Такая строка помечена «через шлюз, адрес клиента не виден», и метка
«локальная сеть» рядом с ней не ставится.

Шлюз узнаётся по числу разных учёток за одним внутренним адресом: за
рабочим местом один человек, за шлюзом вся сеть. Порог —
ZIMBRA_GATEWAY_MIN_ACCOUNTS, по умолчанию 3.

Фразы «снаружи не заходил никто» здесь нет намеренно: при NAT это
неправда, а без NAT — утверждение о том, чего в журнале нет.

Настоящее решение — на стороне Zimbra: прописать адреса прокси в
zimbraMailTrustedIP, тогда в лог попадёт адрес клиента, а не шлюза,
и станут видны страны и подбор паролей по настоящим адресам.

━━━━━━━━━━━━━━━━━━━━
⚠️ ГДЕ ФИЛЬТРОВ НЕТ

Только в обзоре дашборда и только для успешных входов. Неудачные
попытки видны все: подбор пароля из своей же сети означает
заражённую машину внутри периметра. Полные списки без единого
фильтра — в карточке сервера, 📬 Почта → 📤 Отправители и 🔑 Входы.
"""
...
Клетки сервера: сколько попыток поймано, сколько адресов забанено
сейчас и всего, и сами адреса со странами. Плюс белый список
fail2ban (ignoreip) — адреса оттуда он не забанит никогда.

Отдельно — кого банили за сутки. Статус знает только тех, кто
забанен ПРЯМО СЕЙЧАС, а при часовом bantime список почти всегда
пуст: «заблокировано 0» рядом с «всего 66» выглядит поломкой, хотя
оба числа верны. История читается из лога fail2ban, если он
доступен учётке (обычно группа adm).

━━━━━━━━━━━━━━━━━━━━
🟠 КАНДИДАТЫ

Порог maxretry считает попытки С ОДНОГО адреса. Распылённый перебор
идёт по одной-две попытки с каждого из десятков адресов и под порог
не попадает никогда — это свойство признака, а не недонастройка.

Кандидаты берутся из почтовых находок: те же адреса, по которым
приходят алерты о переборе. Уже забаненные и адреса из белого списка
в список не идут — предлагать заблокировать заблокированное значит
тратить ваше время впустую.

Внутренние адреса не предлагаются никогда: за ними шлюз или рабочее
место, и блокировка отрежет своих.

━━━━━━━━━━━━━━━━━━━━
🚫 БЛОКИРОВКА

Кнопкой, по одному адресу. Бот сам не блокирует никого — как и на
Windows: ошибка в один адрес отрезает офис или филиал.

Кнопка «🛡 Заблокировать» есть и ПРЯМО В АЛЕРТЕ о переборе пароля:
находка называет адреса, и переносить их сюда руками не нужно. Она
спрашивает подтверждение, потом банит — на Linux через fail2ban, на
Windows правилом фаервола. Внутренние адреса в такие кнопки не
попадают: банится сразу, а за приватным адресом свои.

Срок ручного бана такой же, как у автоматического (bantime клетки),
и снимается он сам. Ручные баны идут в клетку с почтовыми портами:
основной перебор там, и разбан потом искать в одной клетке, а не в
двух.

Правами управляет TELEGRAM_DELETE_USERS — те же, кто может менять
конфиг и перезагружать серверы.

━━━━━━━━━━━━━━━━━━━━
🔐 ЧТО НУЖНО НА СЕРВЕРЕ

fail2ban-client работает через сокет, доступный только root. Учётке
SSH нужно правило sudo без пароля на четыре формы команды: status,
get ignoreip, set banip, set unbanip. Останавливать fail2ban или
менять его настройки она при этом не может.

Без правила раздел не сломается — покажет текст ошибки вместо
пустого списка, который читался бы как «никто не заблокирован».

Внутренние сети обязаны быть в ignoreip самого fail2ban: если люди
ходят в почту через шлюз, все ошибки приходят с его адреса, и
несколько чужих попыток забанят офис целиком.
"""
...
HELP_SECTIONS = {
    ...
    "copyfail": ("🚑 Копирование: сбои",    HELP_COPY_FAIL),
    ...
    "zimbradash": ("📮 Zimbra в дашборде",  HELP_ZIMBRA_DASH),
    "iis":     ("🌐 IIS",                  HELP_IIS),
    "geo":     ("🌍 Страна по IP",        HELP_GEO),
    "firewall": ("🛡 Блокировка IP",     HELP_FIREWALL),
    "fail2ban": ("🛡 Блокировка (fail2ban)", HELP_FAIL2BAN),
    "autoban": ("🛡 Автобан перебора почты", HELP_AUTOBAN),
