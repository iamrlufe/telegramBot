...
а кнопка ✅ Принято глушит конкретный алерт на сутки.

Кнопка ✅ Принято под алертом глушит именно его (проблема + объект)
на сутки, а не весь сервер: пригодится, когда причина устранена, а
источник ещё отдаёт старые записи. Список принятых и досрочная
отмена — ⚙️ Настройка → ✅ Принятые алерты, срок — ALERT_ACK_HOURS.
...
• мало места на диске: свободно < 15% / 10% / 5%
  ...
  дисками не считаются: efivars и подобные всегда заполнены.
  Диск без свежих метрик пропадает из отчёта через 2 часа
...
• недельная копия не появилась к своему сроку — в тот же день,
  с допуском 24 ч (BACKUP_WEEKLY_GRACE_HOURS). В расписании
  указывай время, к которому копия ГОТОВА, а не старт задания
