...
📜 Скрипт копирования — бот спросит путь по шагам: сперва для
   ПОЛНОЙ копии (обязательно), потом для РАЗНОСТНОЙ и ЖУРНАЛА —
   эти два пропускаются кнопкой. Пути на самом сервере (.ps1,
   .bat, .cmd, .exe). Типы берутся из самих скриптов.
...
📥 Приёмник копий — бот спросит имя приёмника и каталог на нём,
   сохранит разом. Только ради процента: у SFTP нет обратной связи
   о ходе передачи, сколько доехало, знает лишь приёмник. Он должен
   быть заведён у бота как сервер — размер бот смотрит по WinRM,
   а не по SFTP; короткое имя тоже найдётся. Задал — у едущей
   базы появится шкала: ▰▰▰▱▱▱▱▱▱▱ 32%, 14.26 ГБ, ещё ≈2 ч.
