...
📥 Приёмник копий — бот спросит две вещи подряд: имя приёмника
   ...
   готовности. Приёмник должен быть заведён у бота как сервер
   (⚙️ Настройка → ➕ Добавить сервер): размер файла бот смотрит
   по WinRM, а не по SFTP. Короткое имя тоже найдётся. У SFTP нет обратной связи о ходе передачи: сколько
