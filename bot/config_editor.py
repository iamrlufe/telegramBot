...
опрос по WinRM. Для журнала Security (раздел 🔐 Входы) её нужно добавить
в локальную группу Event Log Readers на сервере — иначе раздел пуст, и
это выглядит как «атак не было». Раздел сам напоминает об этом."""
...
Agent: что именно и где не сработало. Частый случай — путь на букве
...
Эти разделы показывают «сейчас», период к ним не применяется.
