...
🔎 Сканирование — запросы мимо публикаций, кто стучится и главное:
  ответил ли сервер успехом. Ответы 200 на «/» и 301 с порта 80
  находками не считаются: это стартовая страница IIS и редирект на
  HTTPS, они отдаются на любой путь.
...
💥 Ошибки 5xx — ошибки самого приложения, а не IIS.
🐢 Медленные — запросы дольше IIS_SLOW_MS (10 секунд).
📚 Публикации — с трафиком и без. Публикация без трафика это открытая
  наружу точка входа без присмотра.
