...
в кодировке консоли (866 или 1251), а WinSCP при перенаправлении —
в UTF-8. В алерте бот разбирает строки поодиночке и читает все три,
менять что-то в скрипте ради этого не нужно.

Если хочется, чтобы и файл на диске был однородным: chcp 65001 > nul
И сам .cmd сохранить в UTF-8 без BOM — иначе строки echo останутся
в старой кодировке и смесь просто поменяется местами. Двух chcp
подряд быть не должно: работает последний.
