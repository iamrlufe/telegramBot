...
def _sparkline(ax, times, values, color, ylim=(0, 100)):
    """Мини-график без осей: линия, заливка, точка со значением."""
    ax.set_facecolor(SURFACE)
    ax.set_xticks([])
    ax.set_yticks([])
    ...
    if not times:
        ...
        return
    ax.plot(times, values, color=color, linewidth=1.6, zorder=3)
    ax.fill_between(times, values, color=color, alpha=0.14, zorder=2)
    ax.plot(times[-1], values[-1], "o", markersize=4, color=color,
            markeredgecolor=SURFACE, markeredgewidth=0.8, zorder=4)
    ax.set_ylim(*ylim)
    last = round(values[-1])
    value_color = STATUS_CRIT if last >= 90 else STATUS_WARN if last >= 80 else INK
    ax.text(1.02, 0.5, f"{last}%", transform=ax.transAxes,
            ha="left", va="center", fontsize=10, fontweight="bold", color=value_color)


def _disk_bar(ax, disk_name, free_pct):
    """Горизонтальная полоска худшего диска: занятое место."""
    ax.set_facecolor(SURFACE)
    ax.set_xticks([])
    ax.set_yticks([])
    for spine in ax.spines.values():
    ...
    color = STATUS_CRIT if free_pct < 10 else STATUS_WARN if free_pct < 20 else SERIES[1]
    ax.barh([0.22], [100], height=0.42, color=GRID, zorder=1)
    ax.barh([0.22], [used_pct], height=0.42, color=color, zorder=2)
    ax.set_xlim(0, 100)
    ax.set_ylim(-0.75, 0.75)
    # Подпись под полоской, чтобы не пересекаться с заголовком колонки
    ax.text(0.5, 0.08, f"{disk_name}: свободно {round(free_pct)}%",
            transform=ax.transAxes, ha="center", va="center",
            fontsize=8.5, color=INK_SECONDARY)


def build_dashboard_chart(hours: int = 24) -> str:
    """
    Одна картинка по всей инфраструктуре: строка на сервер —
    статус, спарклайны CPU/RAM за сутки, худший диск.
    """
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("""
    ...
    history = defaultdict(lambda: {"cpu_t": [], "cpu": [], "ram_t": [], "ram": []})
    for server_name, checked_at, cpu_load, ram_total, ram_free in history_rows:
        t = _to_local(checked_at)
        entry = history[server_name]
        if cpu_load is not None:
            entry["cpu_t"].append(t)
            entry["cpu"].append(float(cpu_load))
        if ram_total and ram_free and float(ram_total) > 0:
            entry["ram_t"].append(t)
            entry["ram"].append(round((float(ram_total) - float(ram_free)) / float(ram_total) * 100, 1))
    ...
    for server_name, disk_name, free_gb, used_gb in disk_rows:
        ...
        current = worst_disk.get(server_name)
        if current is None or free_pct < current[1]:
            worst_disk[server_name] = (disk_name, free_pct)

    now_utc = datetime.now(timezone.utc)
    stale_before = now_utc - timedelta(minutes=DASHBOARD_STALE_MINUTES)

    def sort_key(row):
        name, status, checked_at = row
        is_down = status != "online"
        is_stale = checked_at.replace(tzinfo=timezone.utc) < stale_before
        return (0 if is_down else 1 if is_stale else 2, name.lower())

    rows = sorted(latest_rows, key=sort_key)
    n = len(rows)

    fig = plt.figure(figsize=(11, 0.9 + 0.62 * n))
    fig.patch.set_facecolor(SURFACE)
    gs = gridspec.GridSpec(
        n, 4, figure=fig,
        width_ratios=[2.6, 2.1, 2.1, 1.9],
        left=0.02, right=0.97, top=1 - 0.55 / (0.9 + 0.62 * n),
        bottom=0.3 / (0.9 + 0.62 * n), hspace=0.55, wspace=0.28,
    )
    fig.suptitle(f"Дашборд · {n} серверов · {period_label(hours)}",
                 fontsize=15, fontweight="bold", color=INK)

    for i, (server_name, status, checked_at) in enumerate(rows):
        checked_utc = checked_at.replace(tzinfo=timezone.utc)
        is_stale = checked_utc < stale_before
        if status == "online" and not is_stale:
            dot_color, status_text = STATUS_GOOD, "онлайн"
        elif status == "online" and is_stale:
            dot_color, status_text = STATUS_WARN, "нет свежих данных"
        else:
            dot_color, status_text = STATUS_CRIT, status
        ...
        for spine in ax.spines.values():
            spine.set_visible(False)
        ax.text(0.02, 0.62, f"● {server_name}", fontsize=11.5, fontweight="bold",
                color=dot_color, transform=ax.transAxes, va="center")
        ax.text(0.06, 0.18, status_text, fontsize=8.5,
                color=INK_SECONDARY, transform=ax.transAxes, va="center")

        entry = history.get(server_name, {"cpu_t": [], "cpu": [], "ram_t": [], "ram": []})

        # CPU и RAM спарклайны
        ax_cpu = fig.add_subplot(gs[i, 1])
        _sparkline(ax_cpu, entry["cpu_t"], entry["cpu"], CPU_COLOR)
        ax_ram = fig.add_subplot(gs[i, 2])
        _sparkline(ax_ram, entry["ram_t"], entry["ram"], RAM_COLOR)

        # Худший диск
        ax_disk = fig.add_subplot(gs[i, 3])
        disk = worst_disk.get(server_name)
        _disk_bar(ax_disk, disk[0] if disk else "", disk[1] if disk else None)

        if i == 0:
            ax_cpu.set_title("CPU", fontsize=9, color=INK_SECONDARY, pad=4)
            ax_ram.set_title("RAM (занято)", fontsize=9, color=INK_SECONDARY, pad=4)
            ax_disk.set_title("Худший диск", fontsize=9, color=INK_SECONDARY, pad=6)

    generated_at = datetime.now(ALMATY).strftime("%d.%m.%Y %H:%M")
    fig.text(0.02, 0.006, f"AgentMonitor · сформировано {generated_at}",
             fontsize=8.5, color=INK_MUTED)

    return _save_figure(fig, "dashboard_")
