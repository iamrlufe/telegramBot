...
# Сколько ждать копию после появления файла на источнике. Копирование
# крупной базы занимает минуты, а не секунды, и запускается не мгновенно
# после создания файла — без задержки тревога звенела бы каждую ночь.
MIRROR_LAG_MINUTES = int_env("BACKUP_MIRROR_LAG_MINUTES", 45)
